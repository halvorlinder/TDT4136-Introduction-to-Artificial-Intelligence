...
class CSP:
    def __init__(self, var_select='first'):
        ...
        # string that specifies how to choose the next var
        self.var_selct = var_select
    ...
    def backtrack(self, assignment):
        ...
        # keep track of calls
        self.calls += 1
        # if all vars are assigned, return the consistent assignment
        if (len(list(filter(lambda var: len(var) != 1, assignment.values())))) == 0:
            return assignment
        # get a variable to assign a value to 
        var = self.select_unassigned_variable(assignment)
        # iterate over all values in its domain 
        for value in assignment[var]:
            curr_assignment = copy.deepcopy(assignment)
            if True: # The pseudo code has the check "value is consitent with assignment". What does this do?
                # update assignment with new value for var
                curr_assignment[var] = [value]
                inferences = self.inference(
                    curr_assignment, self.get_all_arcs())
                if inferences:
                    result = self.backtrack(curr_assignment)
                    if result:
                        return result
        self.fails += 1
        return None
    ...
    def select_unassigned_variable(self, assignment):
        ...
        """
        # simply return the first unassigned variable
        if self.var_selct=='first': return list(filter(lambda var: len(var[1]) > 1, assignment.items()))[0][0]
        # return the var with the smallest domain
        if self.var_selct=='mrv': return sorted(list(filter(lambda var: len(var[1]) > 1, assignment.items())), key=lambda var: len(var[1]))[0][0]
    ...
    def inference(self, assignment, queue):
        ...
        """
        # do untill queue is empty
        i = 0
        while i < len(queue):
            # pop arc
            (a, b) = queue[i]
            # if revise alters the domain
            if self.revise(assignment, a, b):
                # if the domain is now empty indicate failure
                if len(assignment[a]) == 0:
                    return False
                # else append all neighbours to the queue
                for (x, _) in self.get_all_neighboring_arcs(a):
                    if x != a and x != b:
                        queue.append((x, a))
            i += 1
        return True
    ...
    def revise(self, assignment, i, j):
        ...
        """
        revised = False
        # create a new list for the domain to avoid deleting while iterating
        x_domain = []
        # iterate over each value in D_i
        for x in assignment[i]:
            exists = False
            # check if there exists a value in D_j that is consistent with x
...
def create_sudoku_csp(filename: str, var_select='first') -> CSP:
...
def do_board(name: string):
    csp = create_sudoku_csp(name, var_select='mrv')
    ans = csp.backtracking_search()
    print(f'Calls (MRV): {csp.calls}')
    print(f'Fails (MRV): {csp.fails}')
    csp = create_sudoku_csp(name, var_select='first')
    ans = csp.backtracking_search()
    print(f'Calls (First): {csp.calls}')
    print(f'Fails (First): {csp.fails}')
    print_sudoku_solution(ans) if ans else print('No solution')
    print()
